...
import { ListarEntidadesApi, CrearEntidadApi, EliminarEntidadApi } from "../api/contactos.api";

export const useEntidades = () => {
  const queryClient = useQueryClient();

  const { data: entidades = [], isLoading: cargandoEntidades } = useQuery({
    queryKey: ["entidades-contactos"],
  ...
  const mutationCrear = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["entidades-contactos"] });
    },
  });
