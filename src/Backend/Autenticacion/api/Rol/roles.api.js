...
export const asignarRolApi = async (data) => {
    const respuesta = await axiosInitial.post(`/roles/asignarRol`, data, { showLoader: false });
    return respuesta.data;
}

export const actualizarRolApi = async (codigo, data) => {
    const respuesta = await axiosInitial.patch(`/roles/actualizar?codigo=${codigo}`, data, { showLoader: false });
    return respuesta.data;
}
