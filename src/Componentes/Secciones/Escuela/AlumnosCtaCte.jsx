...
const KpiCard = ({
  ...
  sub,
  Icon,
  colorCls = "text-[var(--primary)]",
  bgCls = "bg-[var(--primary)]/10",
}) => (
  <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex items-center gap-4">
    <div
      className={`w-10 h-10 rounded-md ${bgCls} flex items-center justify-center shrink-0`}
    >
    ...
    <div className="min-w-0">
      ...
      <p className={`text-lg font-black leading-tight mt-0.5 ${colorCls}`}>
        {value}
      </p>
      {sub && (
...
const AlumnosCtaCte = () => {
  ...
  const pctInternos =
    totalAlumnos > 0 ? Math.round((countInternos / totalAlumnos) * 100) : 0;
  ...
  const maxPagos = useMemo(
    () => Math.max(...ultimosMeses.map((m) => m.pagos), 1),
    [ultimosMeses],
  );
  const maxBarVal = useMemo(
    () => Math.max(...ultimosMeses.map((m) => m.pagos + m.deuda), 1),
    [ultimosMeses],
  );
    ...
    <div className="flex flex-col min-h-screen text-black p-4 gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          label="Deuda Total"
          value={formatARS(
            alumnosProcesados.reduce((s, a) => s + (a.saldoBase || 0), 0),
          )}
          Icon={DollarSign}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col items-center gap-4">
          <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest w-full">
            Distribución
          </span>
        ...
        <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col gap-3">
          <div className="flex justify-between items-start">
            <div>
              ...
              <span className="text-xl font-black text-gray-900 block mt-1 tracking-tight">
          ...
          <svg
            viewBox="0 0 160 50"
            className="w-full h-12 text-[var(--primary)]"
          >
            <path
        ...
        <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col gap-2">
            ...
            {ultimosMeses.map((m, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[9px] font-black text-gray-400 mt-1 uppercase">
          ...
          <div className="flex items-center gap-3 text-[9px] font-black text-gray-400 uppercase">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-sm bg-[var(--primary)] inline-block" />
              Pagado
            </span>
            <span className="flex items-center gap-1">
            ...
            <div className="flex items-center gap-1 bg-black/5 p-1 rounded-xl overflow-x-auto">
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-md text-[11px] font-black uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap ${
                    filtroEstado === tab.id
            ...
            <div className="bg-white border-l border-black/10 w-full max-w-3xl h-full shadow-2xl flex flex-col">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] font-bold text-black/40 uppercase tracking-widest mt-1 block">
                      ...
                      {alumnoSeleccionadoMayor.apellido} (Leg.{" "}
                      {alumnoSeleccionadoMayor.codigo})
                    </span>
                  </div>
                </div>
                <button
                  onClick={handleCerrarLibroMayor}
                  className="p-2 hover:bg-black/5 rounded-full text-black/20 hover:text-black/60 transition cursor-pointer"
                >
                  <X size={20} />
                </button>
              </div>
                ...
                {cargandoMayor ? (
                  ...
                  <div className="p-10 text-center bg-black/5 rounded-2xl border border-dashed border-black/10">
                    ...
                    <p className="text-[11px] font-black text-black/40 uppercase tracking-widest">
                      Sin registros en cuenta {codigoCtaCte}
                    </p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-black/5 p-4 rounded-xl border border-black/5 gap-3">
                      <div>
                        <span className="text-[9px] font-black text-black/40 uppercase tracking-wider block">
                          Saldo Contable
                        </span>
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-1.5 bg-white border border-black/10 rounded-md text-[10px] font-black uppercase tracking-wider transition cursor-pointer hover:bg-black/5 self-start sm:self-auto"
                      >
                        <Printer size={12} />
                        Imprimir Ficha
                      </button>
                    ...
                    <div className="flex items-center gap-3 bg-slate-50 p-4 rounded-xl border border-black/5">
                      ...
                      <select
                        ...
                        className="bg-white border border-black/10 rounded-md px-2 py-1 text-[11px] font-bold outline-none cursor-pointer"
                      >
                        {MESES.map((mes, idx) => (
                          <option key={idx} value={idx}>
                            {mes}
              ...
              <div className="p-6 border-t border-black/5 bg-black/[0.02] flex items-center justify-between">
                <span className="text-[9px] font-black text-black/30 uppercase tracking-widest">
                  General Ledger · EFA Engine
                </span>
                <button
                  onClick={handleCerrarLibroMayor}
                  className="px-6 py-2.5 bg-slate-800 text-white rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-slate-700 transition cursor-pointer"
                >
                  Cerrar
                </button>
              </div>
