...
const ModalReglasCuota = ({
  ...
  const [form, setForm] = useState(
    codigoContactoInicial
      ? {
          ...FORM_VACIO,
          tipoMatch: "CONTACTO",
          codigoContacto: String(codigoContactoInicial),
        }
      : FORM_VACIO,
  );
  ...
  useEffect(() => {
    if (!codigoContactoInicial || cargandoReglas || yaPrecargadoRef.current)
      return;
    yaPrecargadoRef.current = true;
    const existente = reglas?.find(
      (r) =>
        r.tipoMatch === "CONTACTO" &&
        r.codigoContacto === codigoContactoInicial,
    );
    if (existente) {
  ...
  const validar = () => {
    ...
    if (
      ...
      (!form.atributoClave || !form.valorCategoria)
    ) {
  ...
  const handleGuardar = async () => {
    ...
    const dto = {
      ...
      codigoContacto:
        form.tipoMatch === "CONTACTO" ? Number(form.codigoContacto) : undefined,
      atributoClave:
        form.tipoMatch === "CATEGORIA" || form.tipoMatch === "RANGO"
          ? form.atributoClave
          : undefined,
      valorCategoria:
        form.tipoMatch === "CATEGORIA" ? form.valorCategoria : undefined,
      rangoDesde:
        form.tipoMatch === "RANGO" ? Number(form.rangoDesde) : undefined,
      rangoHasta:
        form.tipoMatch === "RANGO" ? Number(form.rangoHasta) : undefined,
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between rounded-t-xl">
          <div>
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">
              Prioridad de match: Contacto específico &gt; Categoría &gt; Rango
              &gt; Default.
            </p>
          </div>
          <button
            ...
            <div className="flex flex-col gap-3 bg-gray-50 border border-gray-200 rounded-md p-5 shadow-sm">
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, tipoMatch: e.target.value }))
                    }
              ...
              {form.tipoMatch === "CONTACTO" && (
                <div className="flex flex-col gap-1.5 mt-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, codigoContacto: e.target.value }))
                    }
                      ...
                      >
                        ...
                        {form.atributoClave &&
                          !camposEntidad.some(
                            ...
                            <option value={form.atributoClave}>
                              {form.atributoClave}
                            </option>
                          )}
                      </select>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      campoAtributoSeleccionado.opciones.length > 0 ? (
                        ...
                        >
                          ...
                          {form.valorCategoria &&
                            !campoAtributoSeleccionado.opciones.includes(
                              form.valorCategoria,
                            ) && (
                              <option value={form.valorCategoria}>
                                {form.valorCategoria}
                              </option>
                            )}
                        </select>
              ...
              <label className="flex items-center justify-between cursor-pointer bg-white border border-gray-200 rounded-md px-4 py-3 mt-2 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                <span className="flex flex-col">
                  ...
                  <span className="text-[10px] text-gray-500">
                    Si el alumno no tiene un tutor cargado, la emisión se
                    rechaza ("No tiene tutor") en vez de facturar a nombre del
                    alumno.
                  </span>
                </span>
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      requiereEnteFacturacion: e.target.checked,
                    }))
                  }
              ...
              <div className="flex gap-2 justify-end pt-3 border-t border-gray-100 mt-2">
                ...
                <button
                  ...
                  disabled={procesando}
                  className="flex items-center gap-2 px-5 py-2 text-sm font-bold text-white rounded-md shadow-sm bg-[#1FAE6D] hover:bg-[#178F58] disabled:opacity-50 cursor-pointer transition-all"
                >
                ...
                <table className="w-full border-collapse text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {reglasFiltradas.map((r) => (
                      ...
                      >
                        <td className="px-5 py-4 text-[11px] font-black tracking-widest text-gray-900 uppercase">
                          {r.tipoMatch}
                        </td>
