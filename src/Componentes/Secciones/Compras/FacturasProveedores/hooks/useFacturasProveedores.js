...
export const useFacturasProveedores = () => {
  ...
  const [tipoBusqueda, setTipoBusqueda] = useState("PRODUCTO"); // PRODUCTO o MATERIA_PRIMA
  ...
  const productUI =
    useProductoUI(filtrosProductos, {
      enabled: tipoBusqueda === "PRODUCTO" && !!filtrosProductos.buscarPorGeneral,
      staleTime: 1000 * 60 * 5,
    }) || {};

  const mpUI =
    useMateriaPrimaUI({
      buscarPorGeneral: filtrosProductos.buscarPorGeneral,
      limite: 10
    }, {
      enabled: tipoBusqueda === "MATERIA_PRIMA" && !!filtrosProductos.buscarPorGeneral,
      staleTime: 1000 * 60 * 5,
    }) || {};

  const productos = tipoBusqueda === "PRODUCTO" 
    ? (Array.isArray(productUI.productos) ? productUI.productos : [])
    : (Array.isArray(mpUI.materiasPrimas) ? mpUI.materiasPrimas : []);

  const cargandoProductos = tipoBusqueda === "PRODUCTO" ? !!productUI.cargando : !!mpUI.cargando;
  ...
  const [manualPuntoVenta, setManualPuntoVenta] = useState("");
  ...
  const siguientePaso = useCallback(() => {
    setPaso((prev) => {
      if (prev === 1 && items.length === 0) {
        agregarAlerta({
          title: "Carrito vacío",
          message: "Agregue productos antes de registrar la compra",
          type: "warning",
        });
        return prev;
      }
    ...
    (e) => {
      if (e.key === "ArrowDown") {
        ...
        if (p) {
          ...
          agregarItem({ ...p, tipoArticulo: tipoBusqueda }, 1, (index) => {
            if (precio <= 0) {
              setTimeout(() => {
  ...
  const confirmarVentaFinal = useCallback(
    async (opciones = {}) => {
      ...
      const body = {
        tipo: 'COMPRA',
        puntoVenta: Number(manualPuntoVenta) || Number(
          fiscal.enBlanco === "si"
        ...
        numeroComprobante: Number(manualNumero) || undefined,
        cae: manualCae || undefined,
        ...
        items: cart.items.map((item) => {
          ...
          return {
            codigoProducto:
              Number(item.codigoSecuencial || item.id_producto || item.id) ||
              undefined,
            ...
            tipoArticulo: item.tipoArticulo || 'PRODUCTO',
          };
        }),
        totales: {
