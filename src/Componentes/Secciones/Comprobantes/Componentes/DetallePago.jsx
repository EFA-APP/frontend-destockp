...
const CODIGO_CUENTA_BANCOS = "1.1.02";

const METODOS = [
  { value: "EFECTIVO", label: "Efectivo", Icon: Banknote, color: "text-emerald-600 bg-emerald-50" },
  { value: "TRANSFERENCIA", label: "Transf.", Icon: ArrowLeftRight, color: "text-sky-600 bg-sky-50" },
  { value: "TARJETA_DEBITO", label: "T. Débito", Icon: CreditCard, color: "text-violet-600 bg-violet-50" },
  { value: "TARJETA_CREDITO", label: "T. Crédito", Icon: CreditCard, color: "text-orange-600 bg-orange-50" },
  { value: "CHEQUE_TERCERO", label: "Ch. Tercero", Icon: FileText, color: "text-amber-600 bg-amber-50" },
  { value: "CHEQUE_PROPIO", label: "Ch. Propio", Icon: FileCheck, color: "text-rose-600 bg-rose-50" },
];
...
const datosTarjetaInit = {
  marca: "", cantidadCuotas: 1, recargo: 0, cupon: "", lote: "", autorizacion: "", fechaAcreditacion: "",
};
const chequeTerceroInit = {
  banco: "", numero: "", cuitEmisor: "", titular: "", fechaEmision: "", fechaPago: "",
};
const chequesPropioInit = {
  tipoCheque: "CORRIENTE", banco: "", sucursal: "", numero: "", cuenta: "", fechaEmision: "", fechaPago: "",
};
...
export const DetallePago = ({ totalComprobante = 0, tipoOperacion = "INGRESO" }) => {
  const { data: bancos = [] } = useObtenerDescendientesImputablesQuery(CODIGO_CUENTA_BANCOS);
  ...
  const handleAgregarPago = () => {
    ...
    const pago = {
      ...
      nombreBanco: bancos.find((b) => b.codigoSecuencial === Number(codigoBanco))?.nombre || "",
      ...(["TARJETA_DEBITO", "TARJETA_CREDITO"].includes(tipoPago) && {
        datosTarjeta: { ...tarjeta, cantidadCuotas: Number(tarjeta.cantidadCuotas), recargo: parseFloat(tarjeta.recargo) || 0 },
      }),
      ...(tipoPago === "CHEQUE_TERCERO" && { chequeTercero: { ...chequeTercero } }),
      ...(tipoPago === "CHEQUE_PROPIO" && { chequePropio: { ...chequePropio } }),
    };
  ...
  const handleAgregarVuelto = () => {
    ...
    setVueltos((prev) => [
      ...prev,
      {
        ...
        nombreBanco: bancos.find((b) => b.codigoSecuencial === Number(bancoVuelto))?.nombre || "",
      },
    ]);
    setMontoVuelto("");
    setBancoVuelto("");
    ...
    <div className="flex flex-col gap-4 px-4 pb-4">
      ...
      <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 flex flex-col gap-3">
        ...
        {isTarjeta && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-gray-100">
            ...
            <InputField
              ...
              onChange={(e) => setTarjeta((p) => ({ ...p, autorizacion: e.target.value }))}
            />
            <InputField
              label="Fecha acreditación"
              type="date"
              value={tarjeta.fechaAcreditacion}
              onChange={(e) => setTarjeta((p) => ({ ...p, fechaAcreditacion: e.target.value }))}
            />
          </div>
        )}
        ...
        {tipoPago === "CHEQUE_TERCERO" && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-2 border-t border-gray-100">
            <InputField label="Banco" type="text" value={chequeTercero.banco}
              onChange={(e) => setChequeTercero((p) => ({ ...p, banco: e.target.value }))} />
            <InputField label="Nro. cheque" type="text" value={chequeTercero.numero}
              onChange={(e) => setChequeTercero((p) => ({ ...p, numero: e.target.value }))} />
            <InputField label="CUIT emisor" type="text" value={chequeTercero.cuitEmisor}
              onChange={(e) => setChequeTercero((p) => ({ ...p, cuitEmisor: e.target.value }))} />
            <InputField label="Titular" type="text" value={chequeTercero.titular}
              onChange={(e) => setChequeTercero((p) => ({ ...p, titular: e.target.value }))} />
            <InputField label="Fecha emisión" type="date" value={chequeTercero.fechaEmision}
              onChange={(e) => setChequeTercero((p) => ({ ...p, fechaEmision: e.target.value }))} />
            <InputField label="Fecha pago" type="date" value={chequeTercero.fechaPago}
              onChange={(e) => setChequeTercero((p) => ({ ...p, fechaPago: e.target.value }))} />
          </div>
        )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-2 border-t border-gray-100">
            <div>
              ...
              <select
                ...
                onChange={(e) => setChequePropio((p) => ({ ...p, tipoCheque: e.target.value }))}
                className="w-full h-[30px] px-2 border border-gray-200 rounded-md text-xs font-bold text-gray-700 bg-white focus:outline-none focus:border-[var(--primary)] cursor-pointer"
              >
            ...
            <InputField label="Banco" type="text" value={chequePropio.banco}
              onChange={(e) => setChequePropio((p) => ({ ...p, banco: e.target.value }))} />
            <InputField label="Sucursal" type="text" value={chequePropio.sucursal}
              onChange={(e) => setChequePropio((p) => ({ ...p, sucursal: e.target.value }))} />
            <InputField label="Nro. cheque" type="text" value={chequePropio.numero}
              onChange={(e) => setChequePropio((p) => ({ ...p, numero: e.target.value }))} />
            <InputField label="Cuenta" type="text" value={chequePropio.cuenta}
              onChange={(e) => setChequePropio((p) => ({ ...p, cuenta: e.target.value }))} />
            <InputField label="Fecha emisión" type="date" value={chequePropio.fechaEmision}
              onChange={(e) => setChequePropio((p) => ({ ...p, fechaEmision: e.target.value }))} />
            <InputField label="Fecha pago" type="date" value={chequePropio.fechaPago}
          ...
          {pagos.map((pago) => (
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                <span className={`shrink-0 flex items-center gap-1 text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider ${badgeColor(pago.tipoMetodoPago)}`}>
                ...
                {pago.datosTarjeta?.marca && (
                  <span className="text-[10px] font-semibold text-gray-500">
                    {pago.datosTarjeta.marca} · {pago.datosTarjeta.cantidadCuotas}c
                  </span>
                )}
                ...
                {pago.chequePropio?.numero && (
                  <span className="text-[10px] font-semibold text-gray-500 truncate">
                    {pago.chequePropio.tipoCheque === "DIFERIDO" ? "Dif." : "Cte."} #{pago.chequePropio.numero}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-sm font-black text-gray-900">{formatPrice(pago.monto)}</span>
                <button
                  type="button"
        ...
        <div className="border border-gray-100 rounded-xl bg-white shadow-sm">
          <div className="px-4 py-3 space-y-1.5">
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Total comprobante</span>
              <span className="text-xs font-bold text-gray-700">{formatPrice(totalComprobante)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Total pagado</span>
              <span className="text-xs font-bold text-gray-700">{formatPrice(totalPagado)}</span>
            </div>
            {vueltos.length > 0 && (
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Total vuelto</span>
                <span className="text-xs font-bold text-gray-700">{formatPrice(totalVuelto)}</span>
              </div>
            )}
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-black text-gray-900 uppercase tracking-wider">
                  {hayExcedente ? "Excedente" : estaCompleto ? "Saldado" : "Pendiente"}
                </span>
              </div>
              <span className={`text-sm font-black ${hayExcedente ? "text-amber-600" : estaCompleto ? "text-emerald-600" : "text-rose-500"}`}>
      ...
      {hayExcedente && (
        <div className="border border-amber-200 bg-amber-50/50 rounded-xl p-4 flex flex-col gap-3">
          ...
          {vueltos.map((v) => (
            <div key={v.id} className="flex items-center justify-between bg-white border border-amber-100 rounded-lg px-3 py-2 gap-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {v.nombreBanco && (
                  <span className="text-[10px] font-semibold text-gray-500">{v.nombreBanco}</span>
                )}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-sm font-black text-amber-700">{formatPrice(v.monto)}</span>
                <button
                  type="button"
                  onClick={() => setVueltos((prev) => prev.filter((x) => x.id !== v.id))}
                  className="p-1 rounded-md text-gray-300 hover:text-red-500 hover:bg-red-50 transition cursor-pointer"
                >
                  <BorrarIcono size={13} />
                </button>
