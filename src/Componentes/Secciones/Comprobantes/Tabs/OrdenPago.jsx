...
const OrdenPago = () => {
  ...
  const currentUnidadNegocio =
    contactoSeleccionado?.codigoUnidadNegocio ||
    usuario?.codigoUnidadNegocio ||
    1;

  const { data: deudasRaw, isLoading: cargandoDeudas } = useQuery({
    queryKey: [
      "deudas-orden-pago",
      contactoSeleccionado?.codigo,
      currentUnidadNegocio,
    ],
    queryFn: () =>
      ObtenerDeudasContactoApi(
        contactoSeleccionado.codigo,
        currentUnidadNegocio,
      ),
  ...
  const totalAplicado = deudas.reduce(
    (s, d) => s + (d.importeAplicado || 0),
    0,
  );
  ...
  const handleConfirmar = () => {
    ...
    const periodoStr = ahora.toLocaleDateString("es-AR", {
      month: "long",
      year: "numeric",
    });
    ...
    crearComprobante(
      {
        dto: {
          ...
          total: montoFinal,
          detalle: (() => {
            const deudasSeleccionadas = deudas.filter(
              (d) =>
                seleccionados.has(d.codigo) && (d.importeAplicado || 0) > 0,
            );
            const items = deudasSeleccionadas.map((d) => ({
              tipoDetalle: "CUENTA_CONTABLE",
              codigoDetalle: 0,
              descripcion:
                `Pago a cuenta ${d.tipoDescripcionComprobante || "comprobante"} Nro ${d.numeroComprobante || ""}`.trim(),
              cantidad: 1,
            ...
            const montoAsignado = deudasSeleccionadas.reduce(
              (sum, d) => sum + (d.importeAplicado || 0),
              0,
            );
            ...
            if (saldoAdicional > 0 || items.length === 0) {
              items.push({
                ...
                cantidad: 1,
                precioUnitario:
                  saldoAdicional > 0 ? saldoAdicional : montoFinal,
                iva: 0,
          ...
          detallePagos: detallePagos.map((dp) => {
            if (dp.endosoChequeTercero) {
              const { _chequeOriginal, ...endosoLimpio } =
                dp.endosoChequeTercero;
      ...
      },
      {
        onSuccess: (data) => {
          ...
          setVueltos([]);
          setSeleccionados(new Set());
          queryClient.invalidateQueries({ queryKey: ["deudas-orden-pago"] });
        },
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left text-[10px] font-black uppercase tracking-wider text-gray-500">
                        Tipo
                      </th>
                      <th className="px-3 py-2 text-left text-[10px] font-black uppercase tracking-wider text-gray-500">
                        Numero
                      ...
                      <th className="px-3 py-2 text-right text-[10px] font-black uppercase tracking-wider text-gray-500">
                        Total
                      </th>
                      <th className="px-3 py-2 text-right text-[10px] font-black uppercase tracking-wider text-gray-500">
                        Saldo
                      ...
                      return (
                        <tr key={d.codigo ?? idx} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 py-2 font-semibold text-gray-700">
                            {d.tipoDescripcionComprobante}
                          </td>
                          <td className="px-3 py-2 font-semibold text-gray-700">
                            {String(d.puntoVenta ?? 0).padStart(5, "0")}-
                            ...
                            <div className="flex flex-col items-end gap-0.5">
                              <input
                                ...
                                onFocus={() =>
                                  setImportesRaw((prev) => ({
                                    ...prev,
                                    [d.codigo]: d.importeAplicado
                                      ? String(d.importeAplicado)
                                      : "",
                                  }))
                                }
                                onBlur={() => {
                                  const val = parseCurrency(
                                    importesRaw[d.codigo] ?? "",
                                  );
                  ...
                  <tfoot className="bg-gray-50 border-t border-gray-200">
                    <tr>
                      <td
                        colSpan={6}
                        className="px-3 py-2 text-right text-xs font-black uppercase tracking-wider text-gray-700"
                      >
                        Total aplicado:
                      </td>
      ...
      {comprobanteConError && (
        <ModalReintentarComprobante
          ...
          tipoDescripcionComprobante={
            comprobanteConError.tipoDescripcionComprobante
          }
