    ...
    if (accion === "ENTREGAR") {
      ...
      mCobrar.mutate(
        ...
        },
        {
          onSuccess: onSuccess(
            esRepresentacion
              ? "Cheque re-presentado y cobrado correctamente"
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-xl shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6">
          <div className="mb-6 p-4 bg-gray-50 border border-gray-100 rounded-md space-y-1">
          ...
          {accion === "ANULAR" ? (
            <div className="flex items-center gap-3 text-sm font-medium text-rose-700 bg-rose-50 p-4 rounded-md border border-rose-100">
              <AlertTriangle size={20} className="shrink-0" />
              ¿Está seguro que desea anular este cheque? Esta acción no se puede
              deshacer.
            </div>
          ) : (
            <form
              id="accion-cheque-propio-form"
              onSubmit={handleSubmit}
              className="space-y-4"
            >
              {esRepresentacion && (
                <div className="flex items-center gap-3 text-sm font-medium text-emerald-700 bg-emerald-50 p-3 rounded-md border border-emerald-100">
                  <AlertTriangle size={18} className="shrink-0" />
                  Este cheque fue RECHAZADO previamente. Al confirmar, se
                  re-presenta y pasa a COBRADO (R56).
                </div>
              )}
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3 rounded-b-xl">
          ...
          <button
            ...
            disabled={isPending}
            className={`flex items-center gap-2 px-5 py-2 text-sm font-bold text-white rounded-md shadow-sm transition-all ${cfg.btnClass} disabled:opacity-50 disabled:cursor-not-allowed`}
          >
