  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-xl shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="rounded-lg border border-blue-100 bg-blue-50/50 p-5 flex flex-col gap-3">
          <div className="flex items-center justify-between text-[14px]">
            <span className="font-semibold text-gray-500">Paciente</span>
            <span className="font-bold text-gray-900">{fila.nombre || "-"}</span>
          </div>
          <div className="flex items-center justify-between text-[14px]">
            <span className="font-semibold text-gray-500">Documento</span>
            <span className="font-bold text-gray-900">{fila.documento || "-"}</span>
          </div>
          <div className="flex items-center justify-between text-[14px]">
            <span className="font-semibold text-gray-500">Plan</span>
            <span className="font-bold text-gray-900">{fila.plan || "-"}</span>
          </div>
          <div className="flex items-center justify-between text-[14px]">
            <span className="font-semibold text-gray-500">Cliente ID</span>
            <span className="font-bold text-gray-900">{fila.id_cliente || "-"}</span>
          </div>
          <div className="h-px bg-blue-100 my-1" />
          <div className="flex items-center justify-between text-[14px]">
            <span className="font-semibold text-gray-500">Base de Cálculo</span>
            <span className="font-bold text-gray-900">${fmt(fila.base_calculo)}</span>
          </div>
          <div className="flex items-center justify-between text-[14px]">
            <span className="font-semibold text-gray-500">Aporte</span>
            <span className="font-bold text-gray-900">${fmt(fila.aporte)}</span>
          </div>
          <div className="flex items-center justify-between text-[14px]">
            <span className="font-semibold text-gray-500">Diferencia</span>
            <span className="font-bold text-gray-900">${fmt(fila.diferencia)}</span>
          </div>
          <div className="h-px bg-blue-100 my-1" />
          <div className="flex items-center justify-between text-[15px]">
            <span className="font-black text-blue-700">Saldo a Favor</span>
            <span className="font-black text-blue-700">${fmt(fila.saldo_a_favor)}</span>
          </div>
        </div>
