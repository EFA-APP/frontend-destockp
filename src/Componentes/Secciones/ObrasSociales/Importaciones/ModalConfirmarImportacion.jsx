...
import SelectorTipoComprobanteCuota, { useTiposComprobanteCuotaPermitidos } from "../../Escuela/GestionCuotas/SelectorTipoComprobanteCuota";
...
const ModalConfirmarImportacion = ({
  ...
  const aEmitir = filasAImportar.filter((f) => f.contactoEncontrado || f.crearComoClie);
  const omitidas = filasAImportar.filter((f) => !f.contactoEncontrado && !f.crearComoClie);

  const montoTotal = aEmitir.reduce((acc, curr) => acc + (curr.total_facturar || 0), 0);
      ...
      <div className="relative w-full max-w-lg bg-white rounded-xl shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          <p className="text-xs font-semibold text-gray-500">
            Obra Social: <strong className="text-gray-700">{obraSocial?.razonSocial || `${obraSocial?.nombre} ${obraSocial?.apellido}`.trim()}</strong>
          </p>
        ...
        <div className="flex flex-col gap-3 p-5 bg-[var(--color-neutral-bg)] rounded-[var(--radius-base)] border border-[var(--color-neutral-border)]">
          <div className="flex items-center justify-between">
            <span className="text-[14px] font-bold text-[var(--color-neutral-text-muted)]">Facturas a generar</span>
            <span className="text-[18px] font-black text-[var(--color-neutral-text-main)]">{aEmitir.length}</span>
          </div>
          <div className="h-px bg-[var(--color-neutral-border)] w-full" />
          <div className="flex items-center justify-between">
            <span className="text-[14px] font-bold text-[var(--color-neutral-text-muted)]">Monto Total</span>
        ...
        {omitidas.length > 0 && (
          <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-[var(--radius-base)]">
            ...
            <p className="text-[13px] text-amber-800 font-semibold">
              {omitidas.length} {omitidas.length === 1 ? "fila se omitirá" : "filas se omitirán"} (paciente no
              encontrado, sin autorización de alta).
            </p>
          </div>
        )}
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3 rounded-b-xl">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 text-sm font-bold text-white rounded-lg shadow-sm bg-[#1FAE6D] hover:bg-[#178F58] disabled:opacity-50 transition-all cursor-pointer"
          >
