...
import { EditarIcono, BuscadorIcono, ArcaIcono } from "../../../../assets/Icons";
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {isLoading ? (
                        ...
                        <div className="col-span-full py-20 bg-white/[0.01] border border-dashed border-white/5 rounded-md flex flex-col items-center justify-center text-white/10">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest">No se encontraron materias primas</p>
                        </div>
                    ) : (
                        filteredItems.map((item) => (
                            <div
                                key={item.codigoSecuencial}
                                onClick={() => navigate(`/panel/inventario/materia-prima/${item.codigoSecuencial}/editar`, { state: { materiaPrima: item } })}
                                className="group bg-[var(--surface)] border border-white/5 rounded-md p-4 flex items-center justify-between transition-all duration-300 cursor-pointer hover:shadow-lg hover:border-emerald-500/30 hover:shadow-emerald-500/5 active:scale-[0.98]"
                            >
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-md bg-emerald-600/10 text-emerald-500 border border-emerald-500/10 group-hover:bg-emerald-600 group-hover:text-white flex items-center justify-center transition-all">
                                        <ArcaIcono size={18} />
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2 mb-0.5">
                                            ...
                                            <span className="text-[9px] font-black text-emerald-500/50 uppercase tracking-widest">MATERIA PRIMA</span>
                                        </div>
