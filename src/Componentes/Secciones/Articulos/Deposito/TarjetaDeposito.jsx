...
import { MapPin, Settings2, Package, MoreVertical, Building2 } from "lucide-react";
import { PersonaIcono, CheckIcono, TelefonoIcono, VentasIcono } from "../../../../assets/Icons";
        ...
        <div className="group relative bg-[var(--surface)] border border-white/5 rounded-md p-4 transition-all duration-500 hover:shadow-2xl hover:border-white/10 overflow-hidden flex flex-col h-full active:scale-[0.98] cursor-default">
            ...
            <div className="flex flex-row md:flex-col gap-4 flex-grow relative z-10">
                ...
                <div className="flex flex-col items-center justify-start gap-2">
                    <div className={`w-14 h-14 md:w-16 md:h-16 rounded-md bg-black/40 flex items-center justify-center border ${config.border} transition-all duration-500 group-hover:scale-105 ${config.glow}`}>
                ...
                <div className="flex-1 flex flex-col justify-between md:justify-start">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        <div className="inline-flex items-center gap-2 text-white/40 text-[11px] font-bold bg-white/5 px-2 py-1 rounded border border-white/5">
                            <PersonaIcono size={10} className="text-[var(--primary)] opacity-60" />
                            <span className="truncate max-w-[80px]">{suc.responsable || "S/D"}</span>
                        </div>
                        <div className="inline-flex items-center gap-2 text-white/40 text-[11px] font-bold bg-white/5 px-2 py-1 rounded border border-white/5">
                            <MapPin size={10} className="text-[var(--primary)] opacity-60" />
                            <span className="truncate max-w-[80px]">{suc.ciudad || "S/D"}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="border-t border-white/5 pt-4 mt-4 space-y-2 relative z-10">
                <div className="flex items-start gap-3 text-[12px] group-hover:text-[var(--primary-light)] font-medium text-white/60 transition-colors">
                    <VentasIcono size={14} color="var(--primary)" className="opacity-40" />
                    <p className="line-clamp-1 truncate">{suc.direccion || "Sin dirección física registrada"}</p>
                </div>
                <div className="flex items-center gap-3 text-[12px] group-hover:text-[var(--primary-light)] font-medium text-white/60 transition-colors">
                    <TelefonoIcono size={14} color="var(--primary)" className="opacity-40" />
            ...
            <div className="mt-6 flex items-center justify-between relative z-10">
                <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-white/5 border border-white/5">
                    <div className={`w-1.5 h-1.5 rounded-full ${config.badgeBg} animate-pulse shadow-sm`} />
                    <span className="text-[9px] font-black uppercase text-white tracking-widest leading-none pt-0.5">{suc.principal ? "Principal" : "Sucursal"}</span>
                </div>
                ...
                >
                    <Settings2 size={13} className="opacity-40 group-hover/edit:opacity-100 group-hover/edit:rotate-90 transition-all duration-500" />
                    Configurar
                </button>
            </div>

            {/* Subtle glow effect */}
            <div className={`absolute -bottom-10 -right-10 w-24 h-24 ${config.bg} rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700`} />
        </div>
    );
};
