    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.headerLeft}>
          <Text style={styles.emisorMainTitle}>
            {emisor.nombreComercial.toUpperCase()}
          </Text>
          {conexionArca && (
            <>
              <Text style={styles.emisorDetailLabel}>
                Razón Social:{" "}
                <Text style={styles.emisorDetailValue}>
                  {emisor.razonSocial}
                </Text>
              </Text>
              <Text style={styles.emisorDetailLabel}>
                Domicilio Comercial:{" "}
                <Text style={styles.emisorDetailValue}>{emisor.domicilio}</Text>
              </Text>
              <Text style={styles.emisorDetailLabel}>
                Condición frente al IVA:{" "}
                <Text style={styles.emisorDetailValue}>
                  {mapearCondicionIva(emisor.condicionIva)}
                </Text>
              </Text>
            </>
          )}
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.codComprobante}>
            COD. {String(tipoDocumento || "011").padStart(3, "0")}
          </Text>
        </View>
        ...
        <View style={styles.headerRight}>
          <Text style={styles.voucherTitle}>
            {obtenerTituloComprobante(tipoDocumento, fiscal)}
          </Text>
          <View style={styles.flexRow}>
            <Text style={styles.labelBold}>
              Punto de Venta: {ptoVtaFormateado}
            </Text>
            <Text style={styles.labelBold}>Comp. Nro: {nroFormateado}</Text>
          </View>
          <Text style={styles.emisorDetailLabel}>
            Fecha de Emisión:{" "}
            <Text style={styles.emisorDetailValue}>{fechaStr}</Text>
          </Text>
          {conexionArca && (
            <>
              <Text style={styles.emisorDetailLabel}>
                CUIT:{" "}
                <Text style={styles.emisorDetailValue}>{emisor.cuit}</Text>
              </Text>
              <Text style={styles.emisorDetailLabel}>
                Ingresos Brutos:{" "}
                <Text style={styles.emisorDetailValue}>{emisor.iibb}</Text>
              </Text>
              <Text style={styles.emisorDetailLabel}>
      ...
      <View style={styles.receptorSection}>
        <View style={styles.receptorLeft}>
          ...
          <Text style={styles.emisorDetailLabel}>
            ...
            <Text style={styles.emisorDetailValue}>
              {mapearCondicionIva(
                receptor?.CondicionIVAReceptorId || receptor?.condicionIva,
              )}
            </Text>
          </Text>
          <Text style={styles.emisorDetailLabel}>
            Condición de venta:{" "}
            <Text style={styles.emisorDetailValue}>
              {comprobante.condicionVenta?.toUpperCase() || "CONTADO"}
            </Text>
          </Text>
        </View>
        <View style={styles.receptorRight}>
          <Text style={styles.emisorDetailLabel}>
            ...
            <Text style={styles.emisorDetailValue}>
              {receptor?.razonSocial || "CONSUMIDOR FINAL"}
            </Text>
          </Text>
          <Text style={styles.emisorDetailLabel}>
            Domicilio:{" "}
      ...
      <View style={styles.summaryContainer}>
          ...
          {((cbtesAsoc && cbtesAsoc.length > 0) ||
            ...
            <View>
              ...
              {cbtesAsoc?.map((cb, i) => (
                <Text
                  key={`asoc-${i}`}
                  style={{ fontSize: 7, marginBottom: 2 }}
                >
                  TIPO: {cb.tipo} | PTO. VTA:{" "}
                  {String(cb.ptoVta).padStart(4, "0")} | NRO:{" "}
                  {String(cb.nro).padStart(8, "0")}
                </Text>
              ))}
              ...
              {ajustes?.map((aj, i) => {
                ...
                return (
                  <Text
                    ...
                    style={{ fontSize: 7, marginBottom: 2 }}
                  >
                    {label}: {String(aj.ptoVta).padStart(4, "0")}-
                    {String(aj.nro).padStart(8, "0")} | {signo}$
                    {aj.total?.toLocaleString("es-AR", {
                      minimumFractionDigits: 2,
                    })}
                  </Text>
                );
        ...
        <View style={styles.totalesBox}>
          ...
          <View style={[styles.totalRow, { fontSize: 12 }]}>
            ...
            <Text>
              {(total + recargo).toLocaleString("es-AR", {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </Text>
        ...
        <>
          <View style={styles.footerSection}>
            ...
            <View style={styles.footerRight}>
              ...
              <Text style={styles.caeLabel}>
                Fecha de Vto. de CAE: {formatearFechaAfip(vtoCae)}
              </Text>
            </View>
          </View>
...
const ComprobantePDF = ({ comprobante, usuario }) => {
  ...
  const listaComprobantes = Array.isArray(comprobante)
    ? comprobante
    : [comprobante];

  return (
    <Document>
      {listaComprobantes.map((comp, idx) => {
        const fiscal = !!comp.cae;
        return (
