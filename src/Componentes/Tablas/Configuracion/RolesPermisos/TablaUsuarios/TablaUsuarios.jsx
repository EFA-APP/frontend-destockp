...
const TablaUsuarios = ({ onAsignarRol }) => {
  ...
  // Mutations
  const {
    mutate: actualizarEstado,
    isPending: isPendingEstado,
    variables: variablesEstado,
  } = useActualizarEstadoUsuario();
  const {
    mutate: eliminarUsuario,
    isPending: isPendingEliminar,
    variables: variablesEliminar,
  } = useEliminarUsuario();
  ...
  const handleToggleEstado = (fila) => {
    ...
    setConfirmData({
      ...
      colorConfirmar:
        accion === "bloquear" ? "bg-amber-600!" : "bg-emerald-600!",
      onConfirm: () => {
        actualizarEstado(
        ...
        <DataTable
          ...
          elementosSuperior={
            <button
              ...
              className="hidden md:flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-600 to-amber-700 hover:to-amber-400 border border-amber-700/20 rounded-md text-[13px] font-bold text-black uppercase tracking-wider  cursor-pointer shadow-lg shadow-amber-700/5 group active:scale-95"
            >
              <UserPlus size={14} className="group-hover:rotate-12 " />
              Agregar Usuario
            </button>
          }
          mobileFab={{
