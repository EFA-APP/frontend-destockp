...
import { Package, X, Database, TrendingUp, TrendingDown, Info, ArrowRight, ArrowLeftRight } from "lucide-react";
...
const DrawerActualizarStock = ({ isOpen, onClose, fila, depositosRaw, depositoInicial }) => {
    ...
    const { mutate: actualizarStock, isPending: isPendingAjuste } = useActualizarStock();
    const { mutate: transferirStock, isPending: isPendingTransfer } = useTransferirStock();
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setDepositoOrigen(depositoInicial || depositosRaw?.[0]?.codigoSecuencial?.toString() || "");
            setDepositoDestino("");
            setCantidad("");
            ...
            actualizarStock(
                {
                    ...
                    codigoUsuario: usuario?.codigoSecuencial ? Number(usuario.codigoSecuencial) : undefined,
                    nombreUsuario: `${usuario?.nombre || ""} ${usuario?.apellido || ""}`.trim() || undefined,
            ...
            `}>
                ...
                <div className="px-6 py-5 border-b border-[var(--border-subtle)] bg-white/[0.02] flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            ...
                            <h2 className="text-[11px] text-[var(--primary-light)] font-medium uppercase tracking-widest mt-0.5">Operación de Stock</h2>
                        </div>
                    </div>
                    <button 
                        onClick={onClose} 
                ...
                <div className="px-6 py-3 border-b border-white/5 bg-black/20 flex gap-2 shrink-0">
                    <button
                        ...
                        className={`flex-1 py-2 rounded-lg text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all ${modo === "ajuste" ? "bg-amber-500/20 border border-amber-500/30 text-amber-500 shadow-md" : "text-white/40 hover:bg-white/5 border border-transparent"}`}
                    >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col gap-6">
                    ...
                    <form id="stock-drawer-form" onSubmit={handleSubmit} className="flex flex-col gap-5">
                            ...
                            <>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        ...
                                        >
                                            <option value="" disabled className="bg-[var(--surface)] text-white/50">Seleccione origen...</option>
                                            {depositosRaw?.map(dep => (
                                                <option key={dep.codigoSecuencial} value={dep.codigoSecuencial} className="bg-[var(--surface)] text-white">
                                                    {dep.nombre}
                                                </option>
                                    ...
                                    <span className="text-[10px] text-white/40 ml-1">Stock actual en origen: <b className="text-white">{stockOrigenActual}</b></span>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        ...
                                        >
                                            <option value="" disabled className="bg-[var(--surface)] text-white/50">Seleccione destino...</option>
                                            {depositosRaw?.map(dep => (
                                                <option key={dep.codigoSecuencial} value={dep.codigoSecuencial} disabled={dep.codigoSecuencial?.toString() === depositoOrigen} className="bg-[var(--surface)] disabled:opacity-30 text-white">
                                                    {dep.nombre}
                                                </option>
                                            ))}
                                        </select>
                        ...
                        <div className="space-y-2">
                            <label className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-widest ml-1 flex justify-between items-end">
                                <span>Cantidad a {modo === "ajuste" ? "Modificar" : "Transferir"}</span>
                            </label>
                            <div className={`relative flex items-center justify-center bg-black/40 border rounded-xl p-2 transition-all focus-within:border-amber-500/50 ${modo === 'transferencia' ? 'border-blue-500/10 focus-within:border-blue-500/50' : 'border-white/10'}`}>
                                <div className={`px-4 text-3xl font-black ${modo === 'transferencia' ? 'text-blue-400' : tipoAjuste === 'agregar' ? 'text-emerald-500' : 'text-red-500'}`}>
                                    {modo === 'transferencia' ? '⇋' : tipoAjuste === 'agregar' ? '+' : '-'}
                                </div>
                                <input 
                                    type="number" 
                                    value={cantidad}
                            ...
                            {modo === 'transferencia' && Number(cantidad) > stockOrigenActual && (
                                <p className="text-[10px] text-red-500 font-bold ml-1">⚠ La cantidad supera el stock actual disponible en el origen.</p>
                            )}
                        </div>
                        ...
                        {modo === "ajuste" && depositoSeleccionado && cantidad && (
                            <div className="flex items-center justify-center gap-4 bg-white/[0.02] p-3 rounded-lg border border-white/5">
                                <span className="text-[10px] uppercase font-bold text-white">Act: <b className="text-[13px] font-black text-amber-500">{stockActualCalculado}</b></span>
                                <ArrowRight size={14} className="text-blue-500" />
                                <span className="text-[10px] uppercase font-bold text-white">Post: <b className={`text-[15px] font-black ${proximoStock < 0 ? 'text-red-500' : 'text-emerald-500'}`}>{proximoStock}</b></span>
                            </div>
                        )}
                        ...
                        <div className="space-y-1 pt-2">
                            <label className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-widest flex items-center gap-1.5 ml-1">
                                <Info size={14} className="text-white/30" /> Observación (Opcional)
                            </label>
                            <textarea 
                                value={observacion}
                ...
                <div className="p-6 md:pb-6 pb-20 border-t border-[var(--border-subtle)] bg-black/40 shrink-0">
                    <button 
                        ...
                        disabled={isPending || !cantidad || (modo === "ajuste" && !depositoSeleccionado) || (modo === "transferencia" && (!depositoOrigen || !depositoDestino || Number(cantidad) > stockOrigenActual))}
                        className={`
                            ...
                            ${!cantidad || (modo === "ajuste" && !depositoSeleccionado) || (modo === "transferencia" && (!depositoOrigen || !depositoDestino || Number(cantidad) > stockOrigenActual))
                    ...
                    >
                        {isPending ? (
                            ...
                            <span>{modo === 'transferencia' ? 'Confirmar Transferencia' : tipoAjuste === 'agregar' ? 'Confirmar Ingreso' : 'Confirmar Egreso'}</span>
                        )}
                    </button>
                </div>
            </div>
