...
import { ImportarContactosApi, ListarConfiguracionCamposApi, ListarEntidadesApi } from "../../../Backend/Contactos/api/contactos.api";
...
import { GuardarIcono, MovimientoIcono, CuentaIcono } from "../../../assets/Icons";
...
const ModalCargaMasivaContactos = ({ open, onClose, onExito }) => {
  ...
  const configActual = useMemo(() => {
    ...
    return configuraciones.filter(c => c.entidadClave === entidadSeleccionada);
  }, [entidadSeleccionada, configuraciones]);

  const descargarPlantilla = () => {
    if (!entidadSeleccionada) {
      setError("Por favor, seleccioná primero qué tipo de contacto vas a cargar.");
      return;
    }
    ...
    configActual.forEach(c => {
      ...
      if (c.tipoDato === "LISTA" && Array.isArray(c.opciones) && c.opciones.length > 0) {
    ...
    XLSX.writeFile(workbook, `plantilla_importar_${entidadSeleccionada.toLowerCase()}.xlsx`);
  };
      ...
      try {
        ...
        const parsedRows = rows.map((row) => {
          const item = {
            ...
            apellido: idxMap.apellido !== -1 ? (row[idxMap.apellido] || "") : "",
            razonSocial: idxMap.razonSocial !== -1 ? (row[idxMap.razonSocial] || "") : "",
            documento: idxMap.documento !== -1 ? (row[idxMap.documento]?.toString() || "") : "",
          ...
          configActual.forEach((c) => {
            ...
            const colIndex = headers.findIndex(h => h.includes(nombreBusqueda));
            ...
            <button
              ...
              className={`p-3 rounded-xl border flex items-center gap-3 transition-all ${
                entidadSeleccionada === ent.clave
                  ? "bg-[var(--primary)]/10 border-[var(--primary)] text-white shadow-[0_0_20px_rgba(var(--primary-rgb),0.1)]"
                  : "bg-white/[0.02] border-white/5 text-white/40 hover:bg-white/5"
              }`}
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                entidadSeleccionada === ent.clave ? "bg-[var(--primary)]/20 text-[var(--primary)]" : "bg-white/5 text-white/20"
              }`}>
                <CuentaIcono size={16} />
              </div>
              <span className="text-xs font-black uppercase tracking-widest">{ent.nombre}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          >
            ...
            <div className="text-left">
              <div className="text-xs font-black uppercase tracking-wider">Planilla {entidadSeleccionada}</div>
              <div className="text-[10px] opacity-60">Descargar Formato .xlsx</div>
            </div>
          </button>

          <label className="flex-1 flex items-center justify-center gap-3 px-6 py-4 bg-[var(--primary)]/10 hover:bg-[var(--primary)]/20 border border-[var(--primary)]/20 rounded-2xl text-[var(--primary)] transition-all cursor-pointer group">
            ...
            <div className="text-left">
              ...
              <div className="text-[10px] opacity-60">Seleccionar Excel listo</div>
            </div>
      ...
      {fileData.length > 0 && (
        <div className="space-y-3 animate-in fade-in duration-500">
          <div className="flex items-center justify-between px-1">
            <label className="text-[11px] font-bold text-white/50 uppercase tracking-widest">Previsualización</label>
            <span className="text-[10px] font-black text-[var(--primary)] uppercase">{fileData.length} registros detectados</span>
          </div>
          
          <div className="space-y-2 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
            {fileData.map((item, idx) => (
              <div key={idx} className="flex items-center gap-4 bg-white/[0.03] p-3 rounded-xl border border-white/5 hover:bg-white/[0.06] transition-all">
                <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center border border-white/10 shrink-0">
                  <CuentaIcono size={14} className="text-white/40" />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[9px] text-white/30 uppercase font-black">{item.documento || "S/D"}</span>
                    <span className="text-[9px] text-white/10">•</span>
                    <span className="text-[9px] text-[var(--primary)]/70 font-bold">{item.condicionIva}</span>
                  </div>
                  {Object.keys(item.atributos).length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {Object.entries(item.atributos).map(([key, val]) => (
                        <span key={key} className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-[8px] font-black text-white/40 uppercase">
                          {key}: {val || '---'}
                        </span>
                      ))}
                    </div>
  ...
  const footer = (
    <div className="flex items-center justify-between w-full">
      <button
        onClick={handleClose}
        className="px-6 py-2.5 text-xs font-black text-white/40 hover:text-white uppercase tracking-widest transition-colors"
      >
        Cancelar
      </button>
      <button
        ...
        className="flex items-center gap-3 px-8 py-2.5 bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 hover:brightness-110 disabled:opacity-30 disabled:grayscale disabled:cursor-not-allowed text-white rounded-xl font-black text-[11px] uppercase tracking-widest transition-all shadow-xl shadow-[var(--primary)]/20 active:scale-95"
      >
        {cargando ? (
