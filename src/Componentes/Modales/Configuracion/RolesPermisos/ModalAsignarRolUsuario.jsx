...
const ModalAsignarRolUsuario = ({ isOpen, onClose, usuario }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/20 backdrop-blur-sm   ">
      <div className="bg-[var(--surface)] w-full max-w-[400px] rounded-md shadow-2xl border border-[var(--border-subtle)] overflow-hidden    relative">
        {/* Overlay de Carga */}
        {asignarRolMutation.isPending && (
          <div className="absolute inset-0 z-[110] bg-[var(--surface)]/80 backdrop-blur-[2px] flex flex-col items-center justify-center   ">
            <div className="flex flex-col items-center gap-3">
              <div className="relative">
                <div className="w-12 h-12 rounded-full border-2 border-[var(--primary)]/20 border-t-[var(--primary)] " />
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-2 h-2 bg-[var(--primary)] rounded-full " />
                </div>
              </div>
              <div className="text-center">
                <p className="text-[15px] font-bold text-[var(--text-primary)] uppercase tracking-widest">
                  Sincronizando
                </p>
                <p className="text-[12px] text-[var(--text-secondary)]">
                  Asignando nuevos roles al sistema...
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-subtle)] bg-[var(--surface-hover)]/30">
          <div>
            <h2 className="text-[16px] font-bold text-[var(--text-primary)]">
              Gestión de Rol Múltiple
            </h2>
            <p className="text-[13px] text-[var(--text-secondary)] mt-0.5">
              Asigne o modifique los permisos del usuario
            </p>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-md text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-active)] "
          >
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="bg-[var(--surface-hover)]/50 p-4 rounded-md border border-[var(--border-subtle)] mb-6 flex items-start gap-4">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[14px] text-[var(--text-secondary)] truncate max-w-[220px]">
                {usuario.correoElectronico}
              </p>
              <div className="pt-1.5 flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-green-700 shadow-[0_0_8px_rgba(34,197,94,0.4)]"></span>
                <span className="text-[12px] uppercase font-bold text-green-700 tracking-wider">
                  Cuenta Activa
                </span>
              </div>
            </div>
          ...
          <div className="space-y-3">
            <label className="text-[14px] font-bold text-[var(--text-primary)] uppercase tracking-wider flex items-center justify-between">
              <span>Roles del Sistema a Asignar</span>
              <span className="text-[12px] font-medium bg-[var(--primary)]/10 text-[var(--primary)] px-2 py-0.5 rounded-sm">
                {rolesSeleccionados.length} Seleccionado
                ...
                roles.map((r) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-3 p-3 rounded-md border text-left cursor-pointer  ${
                        isSelected
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-[15px] font-bold truncate ${isSelected ? "text-[var(--primary)]" : "text-[var(--text-primary)]"}`}
                        >
                          {r.nombre}
                        </p>
                        {r.descripcion && (
          ...
          <div className="flex items-center justify-end gap-3 mt-8 pt-5 border-t border-[var(--border-subtle)]">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 text-[14px] font-bold uppercase tracking-wider text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-hover)] rounded-md  cursor-pointer"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={
                asignarRolMutation.isPending || rolesSeleccionados.length === 0
              }
              className={`px-6 py-2.5 flex items-center gap-2 cursor-pointer bg-gradient-to-r from-[var(--primary)] to-[var(--primary-active)] hover:opacity-90 text-black text-[14px] font-bold uppercase tracking-wider rounded-md shadow-lg shadow-[var(--primary)]/20  transform active:scale-95 ${asignarRolMutation.isPending ? "opacity-70 cursor-wait" : "disabled:opacity-50 disabled:cursor-not-allowed"}`}
            >
              {asignarRolMutation.isPending && (
                <svg
                  className=" -ml-1 mr-2 h-4 w-4 text-black"
