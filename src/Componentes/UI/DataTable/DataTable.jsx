                    ...
                    >
                        ...
                        >
                            ...
                            <div className="truncate">
                                {col.renderizar ? col.renderizar(fila[col.key], fila) : formatVal(fila[col.key])}
                            </div>
                        </div>
                    </td>
                ))}
...
const DataCard = ({
    ...
    const availableStandardActions = [
        { key: 'ver', show: permisos?.ver !== false && onVer, icon: <Eye size={20} className="text-blue-400" />, onClick: onVer },
        { key: 'editar', show: permisos?.editar !== false && onEditar, icon: <Edit size={20} className="text-amber-500" />, onClick: onEditar },
        { key: 'descargar', show: permisos?.descargar !== false && onDescargar, icon: <Download size={20} className="text-emerald-500" />, onClick: onDescargar },
        { key: 'eliminar', show: permisos?.eliminar !== false && onEliminar, icon: <Trash2 size={20} className="text-red-500" />, onClick: onEliminar }
    ].filter(a => a.show);

    const totalActionsCount = acciones ? acciones.length : availableStandardActions.length;
    const isSingleAction = totalActionsCount === 1;
    const singleAction = isSingleAction ? (acciones ? acciones[0] : availableStandardActions[0]) : null;
    ...
    return (
        <>
            ...
            `}>
                ...
                {/* Header: Título Principal + Acciones */}
                <div className="flex items-start justify-between gap-4 relative z-10">
                    <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] animate-pulse" />
                            <span className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">{columnas[0]?.etiqueta}</span>
                        </div>
                    ...
                    {accionesProps && (
                        isSingleAction ? (
                            <button
                                onClick={() => {
                                    if (acciones) {
                                        if (!singleAction.isLoading || !singleAction.isLoading(fila)) {
                                            singleAction.onClick(fila);
                                        }
                                    } else {
                                        singleAction.onClick(fila);
                                    }
                                }}
                                className="w-10 h-10 rounded-xl bg-white/5 text-[var(--text-muted)] flex items-center justify-center border border-white/10 active:scale-95 transition-all"
                                title={singleAction.label || singleAction.etiqueta || "Acción"}
                            >
                                {acciones && singleAction.isLoading && singleAction.isLoading(fila) ? (
                                    <div className="w-4 h-4 border-2 border-[var(--primary)]/20 border-t-[var(--primary)] rounded-full animate-spin" />
                                ) : (
                                    singleAction.icon || singleAction.icono || (React.isValidElement(singleAction.icono) ? React.cloneElement(singleAction.icono, { size: 20 }) : singleAction.icono)
                                )}
                            </button>
                        ) : (
                            <button
                                onClick={() => setActionSheetOpen(true)}
                                className="w-10 h-10 rounded-xl bg-[var(--primary-subtle)]/50 text-[var(--primary)] flex items-center justify-center border border-[var(--primary)]/20 active:scale-95 transition-all shadow-lg shadow-[var(--primary)]/5"
                            >
                ...
                {/* Cuerpo: Atributos Secundarios */}
                <div className="grid grid-cols-2 gap-x-6 gap-y-5 py-4 border-y border-white/5 relative z-10">
                    {columnas.slice(1).map((col) => (
                        <div key={col.key} className="flex flex-col gap-1 overflow-hidden">
                            <span className="text-[8px] font-extrabold text-[var(--text-muted)] uppercase tracking-wider opacity-60">{col.etiqueta}</span>
                            <div className="text-[13px] font-medium text-[var(--text-primary)]/90 truncate">
                                {col.renderizar ? col.renderizar(fila[col.key], fila) : formatVal(fila[col.key])}
                            </div>
                        </div>
                    ))}
                    ...
                    <div className="flex flex-col gap-3 relative z-10">
                        <button
                            ...
                            className={`
                                ...
                                w-full py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest
                                transition-all duration-300
                                ${estaExpandida 
                                    ? 'bg-[var(--primary)] text-white shadow-lg shadow-[var(--primary)]/20' 
                                    : 'bg-white/5 text-[var(--text-muted)] hover:bg-white/10 hover:text-[var(--text-primary)] border border-white/5'}
                            `}
                        >
                            {estaExpandida ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                            {estaExpandida ? 'Ocultar Detalles' : 'Ver más información'}
                        </button>
                        {estaExpandida && (
                            <div className="p-4 rounded-xl bg-black/20 border border-white/5 animate-in slide-in-from-top-4 duration-500 overflow-hidden">
                                {renderDetalle(fila)}
                            </div>
                        )}
                    </div>
            ...
            {actionSheetOpen && accionesProps && (
                <div className="fixed inset-0 z-[10000] flex flex-col justify-end pointer-events-auto">
                    ...
                    <div className="relative bg-[#111111] w-full rounded-t-[40px] border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.6)] flex flex-col animate-in slide-in-from-bottom duration-500 pb-safe pb-8">
                        ...
                        <div className="w-full flex justify-center pt-5 pb-4" onClick={() => setActionSheetOpen(false)}>
                        ...
                        <div className="px-7">
                            <div className="flex justify-between items-center mb-6">
                                <div className="flex flex-col">
                                    <h3 className="text-white font-black text-lg tracking-tight">Acciones</h3>
                                    <p className="text-white/40 text-[10px] uppercase font-bold tracking-widest">Gestionar registro seleccionado</p>
                                </div>
                                <button 
                                    onClick={() => setActionSheetOpen(false)} 
                                ...
                                {acciones ? (
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="w-11 h-11 rounded-xl bg-white/5 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                                                    {cargando ? <div className="w-5 h-5 border-2 border-[var(--primary)]/20 border-t-[var(--primary)] rounded-full animate-spin" /> : (accion.icono && React.isValidElement(accion.icono) ? React.cloneElement(accion.icono, { size: 20, className: "text-[var(--primary)]" }) : accion.icono)}
                                                </div>
                                                <span className="font-bold text-sm tracking-wide">{accion.etiqueta || accion.label || "Acción"}</span>
                                            </button>
                                        );
                                    })
                                ) : (
                                    <>
                                        {availableStandardActions.map((accion) => (
                                            <button 
                                                key={accion.key}
                                                onClick={() => { setActionSheetOpen(false); accion.onClick(fila); }} 
                                            ...
                                            >
                                                <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform bg-white/5`}>
                                                ...
                                                <span className="font-bold text-sm tracking-wide">
                                                    ...
                                                    {accion.key === 'eliminar' && 'Eliminar permanentemente'}
                                                </span>
                                            </button>
                                        ))}
                                    </>
                            ...
                            <button onClick={() => setActionSheetOpen(false)} className="w-full h-14 mt-6 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-white/50 font-black uppercase text-[11px] tracking-[0.2em] hover:bg-white/10 hover:text-white active:scale-[0.97] transition-all mb-4">
                                Cerrar Panel
                            </button>
                        </div>
                    </div>
...
const DataTable = ({
         ...
         setColumnasVisibles(prev => {
              ...
              if (pref && pref.orden && pref.visibles) {
                   ...
                   pref.orden.forEach(key => {
                       ...
                       if (col) nuevas.push({ ...col, visible: pref.visibles.includes(key) });
                   });
                   columnas.forEach(col => {
                       if (!nuevas.some(n => n.key === col.key)) nuevas.push({ ...col, visible: true });
                   });
                   return nuevas;
              }
    ...
    React.useEffect(() => {
        ...
        const timeout = setTimeout(() => {
             const config = {
                 visibles: columnasVisibles.filter(c => c.visible !== false).map(c => c.key),
                                    ...
                                    {opcionesBusqueda && setBusquedaClave && busquedaClave && (
                                        ...
                                        >
                                            {opcionesBusqueda.map(op => (
                                                <option key={op.value} value={op.value} className="bg-[var(--surface)] text-[var(--text-primary)]">
                                                    {op.label}
                                                </option>
                                            ))}
                                        </select>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-1 py-1">
                ...
                {meta && (
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase">Mostrar</span>
                            <select
