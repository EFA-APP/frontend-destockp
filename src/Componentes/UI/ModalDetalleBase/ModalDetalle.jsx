...
const ModalDetalle = ({ title, icon, onClose, children, footer, accentColor = "amber", isStandalone = false }) => {
  ...
  return (
    ...
    >
      ...
      {!isStandalone && (
        <div className={`relative z-10 flex items-center justify-between px-5 py-4 md:px-6 md:py-5 border-b border-white/5 shadow-sm shrink-0`}>
          <div className="flex items-center gap-3.5">
            {icon && (
              <div className={`p-2 rounded-md ${theme.iconBg} ${theme.iconText} border ${theme.iconBorder} shadow-inner`}>
      ...
      {/* Body - Clean & Spaced */}
      <div className={`${isStandalone ? "p-0" : "flex-1 px-4 py-5 md:px-6 md:py-6 overflow-y-auto custom-scrollbar"}`}>
        {children}
      </div>

      {/* Footer - Solid & Formal */}
      {footer && (
        <div className={`px-4 py-4 md:px-6 md:py-5 border-t border-white/5 ${isStandalone ? "bg-black/5" : "bg-black/20"} flex flex-col sm:flex-row justify-end gap-3 shrink-0 rounded-b-md`}>
          {footer}
        </div>
      )}
    </div>
