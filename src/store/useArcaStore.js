...
import { IniciarSesionArcaApi } from "../Backend/Ventas/api/arca.api";
...
export const useArcaStore = create((set) => ({
    ...
    verificarSesion: async (empresa) => {
        ...
        if (!empresa?.conexionArca && !empresa?.configuracionArca?.activo) {
            set({ conectado: false, mensaje: "ARCA no habilitado para esta empresa" });
            return;
        }
        ...
        try {
            ...
            set({ conectado: false, mensaje: "Error de conexión con el servidor ARCA", cargando: false });
        }
    }
}));
